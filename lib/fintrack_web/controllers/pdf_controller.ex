defmodule FinTrackWeb.PdfController do
  ...
  def show(conn, %{"id" => id}) do
    ...
    case generate_invoice_pdf(invoice) do
      {:ok, pdf_binary} ->
        ...
        |> put_resp_header(
          "content-disposition",
          ~s[inline; filename="#{invoice_date(invoice)}-invoice-#{invoice.invoice_number}.pdf"]
        )
  ...
  def download(conn, %{"id" => id}) do
    ...
    case generate_invoice_pdf(invoice) do
      {:ok, pdf_binary} ->
        ...
        |> put_resp_header(
          "filename",
          ~s[invoice_#{invoice.invoice_number}.pdf]
        )
        |> send_resp(200, pdf_binary)
  ...
  defp generate_invoice_pdf(invoice) do
    with {:render_html, html_content} <-
           {:render_html,
            render_invoice_html(invoice, Billing.get_client_info(), Billing.get_bill_to_info())},
         {:ok, base64_pdf} <- ChromicPDF.print_to_pdf({:html, html_content}),
         {:ok, pdf_binary} <- Base.decode64(base64_pdf) do
      {:ok, pdf_binary}
    else
      {:error, reason} ->
        {:error, reason}
      ...
      :error ->
        {:error, "Failed to decode PDF from Base64"}
    end
  rescue
    e ->
        ...
        <table class="services-table">
          ...
          <tbody>
            <tr class="service-row">
              ...
              <td>—</td>
              <td>—</td>
              <td>#{FinTrackWeb.Helpers.FormatHelper.format_currency(invoice.amount)}</td>
            </tr>
  ...
  defp invoice_date(invoice) do
    Calendar.strftime(invoice.date, "%Y%m%d")
  end
end
