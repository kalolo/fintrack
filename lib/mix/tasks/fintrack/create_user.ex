defmodule Mix.Tasks.FinTrack.CreateUser do
  use Mix.Task
  ...
  Create a user account
      ...
      $ mix fintrack.create_user user@example.com password123
  ...
  def run([email, password]) when is_binary(email) and is_binary(password) do
    ...
    case FinTrack.Accounts.create_user(%{email: email, password: password}) do
      {:ok, user} ->
  ...
  def run(_args) do
    Mix.shell().error("Usage: mix fintrack.create_user EMAIL PASSWORD")
  end
end
