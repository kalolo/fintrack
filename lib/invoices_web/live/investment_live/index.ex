defmodule InvoicesWeb.InvestmentLive.Index do
  use InvoicesWeb, :live_view
  import InvoicesWeb.CoreComponents
  alias Invoices.Investments
            ...
            <tbody class="bg-white divide-y divide-gray-100">
              <%= for inv <- @investments do %>
                <tr>
                  ...
                  <td class="px-6 py-4">
                    {format_currency(Invoices.Investments.Investment.previous_value(inv))}
                  </td>
                  <td class="px-6 py-4">
                    <%= if length(inv.value_captures) > 0 do %>
                      {format_currency(Invoices.Investments.Investment.latest_value(inv))}
                    <% else %>
                      --
                    <% end %>
                  </td>
                  <td class="px-6 py-4">
                    <%= if length(inv.value_captures) > 0 do %>
                      ...
                      <% latest = Invoices.Investments.Investment.latest_value(inv) %>
