defmodule FinTrack.Billing do
  ...
  def get_client_info do
    Application.get_env(:fintrack, :invoice_config)[:client]
  end

  def get_bill_to_info do
    Application.get_env(:fintrack, :invoice_config)[:bill_to]
  end
  ...
  defp calculate_payment_method_breakdown(transactions) do
    ...
    |> Enum.map(fn {payment_method, method_transactions} ->
      ...
      %{
        ...
        display_name: FinTrack.Billing.Transaction.payment_method_display(payment_method)
      }
    end)
